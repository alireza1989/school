...
session_start();
$_SESSION['ridddate'] = array();

$con = mysqli_connect("localhost","ams_user14","Cmpt354@123","ams_g14");

//searches for all orders that have not been delivered
$result = mysqli_query( $con, "SELECT receiptId,date,cid,cardnumber,expiryDate,expectedDate FROM Orders WHERE deliveredDate is NULL or deliveredDate = '0000-00-00'");

echo "<form name='viewItem' method='post' action='updatedelivered.php' ><table><tr><th>receiptId</th><th>date</th><th>cid</th><th>cardnumber</th><th>expiryDate</th><th>expectedDate</th><th>deliveredDate</th></tr>";
$i = 0;
		while($row = mysqli_fetch_assoc($result)) {
			echo "<tr><td>".$row['receiptId']."</td><td>".$row['date']."</td><td>".$row['cid']."</td><td>".$row['cardnumber']."</td><td>".$row['expiryDate']."</td><td>".$row['expectedDate']."</td><td><input name='".$i."' type='date' id='".$i."'></td></tr>";

$_SESSION['ridddate'][$row['receiptId']] = $i;
//echo $row['receiptId'] . ", " . $_SESSION['ridddate'][$row['receiptId']];
//echo "<br>";

$i = $i + 1;
		}
		echo "</table>";
echo "<input type='submit' name='selectall' value='Submit All'></form>";
