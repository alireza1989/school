...
//place receiptid, delivereddate pairs into a new array
$tobeupdated = array();
foreach($_SESSION['ridddate'] as $x => $xval){
   if($_POST[$xval] == null){
      //do nothing
   }else{
      $tobeupdated[$x] = $_POST[$xval];
   }
}

//echo "bu hao<br>";

foreach($tobeupdated as $y => $yval){
echo $y . ", " . $yval . "<br>";
}
...
foreach($tobeupdated as $y => $yval){
   $updatedates = "UPDATE Orders SET deliveredDate = '$yval' WHERE receiptId = '$y'";
   //echo $updatedates . "<br>";
   if (mysqli_query($con, $updatedates)) {
