<?php
	...
	if($cid == ''){
		...
		echo '<select name="search" type="select-one"><option value="">Select Option</option><option  value="edit">Edit</option> <option value="delete">Delete</option></select><input name="submit" type="submit" value="Submit"></form><br><form id = "purchase" method = "post" action = "getcard.php"><input name="purchase" type="submit" value="Purchase All"></form> <center><font color=FF3232><h2><b>Error, trying to purchase one or more items greater then the stock.<br>Please check the stock quantity of the item again.</b></h2></font></center>';
	}
