...
$_SESSION['itemupcs'] = array();
$_SESSION['receiptnum'] = $_POST['reciept_id'];
...
// Get data from POST (Clerk html page)
$rID = $_POST['reciept_id'];

//check if receiptid matches
$checkrid = "SELECT * FROM PurchasedItem WHERE receiptId = '$rID'";
//echo $checkrid . "<br>";

if (mysqli_query($con, $checkrid)) {
    echo "query successful";
} else {
    echo "Error executing " . mysqli_error($con);
}

$checkridres = $con->query($checkrid);

if($checkridres->num_rows > 0){
   echo "<form action = 'refundcheck2.php' method='post'><table class = 'report'><tr><td>ReceiptID</td><td>UPC</td><td>quantity bought</td><td>quantity to return</td></tr>";
   while($row = $checkridres->fetch_assoc()){
      $upctracker = $row["upc"];
      $qb = $row["quantity"];
      $_SESSION['itemupcs'][$upctracker] = $upctracker;
      echo"<tr><td>" . $row["receiptId"] . "</td><td>" . $row["upc"] . "</td><td>" . $row["quantity"] . "</td><td>" . "<input type = 'number' name = 'retqty" . $upctracker . "' min='1' max='$qb'>" . "</td></tr>";
   }
